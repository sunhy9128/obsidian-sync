...
| 优先级 | 事项 | 操作 | 状态 |
|--------|------|------|------|
| 🔴 HIGH | 地址计数器漂移 | `--rebuild` | ✅ 已修 |
| 🔴 HIGH | 损坏文件名 | 删 `_1.md`、删双后缀、重命名 | ✅ 已修 |
| 🟡 MEDIUM | 孤儿页 | 跨引用 14 个孤儿页 | ✅ 已修 |
| 🟡 MEDIUM | 文章标题死链 | 4 个 `《》` 别名 | ✅ 已修 |
...
- Address counter rebuild: 959 → 1008 → 1027
- Deleted 3 files, renamed 1
- Fixed 14 orphan pages → 0
- Created 13 stub pages
- Added 5 alias sets

### Round 2 (This Session — "继续干")

#### Article Title Aliases (4)
| 页面 | 新增别名 |
|------|---------|
| [[环球时报]] | `《环球时报》` |
| [[欧盟要与中国打贸易战？]] | `《欧盟要与中国打贸易战？》` |
| [[我们已经处于新一轮加息周期中或前夜]] | `《我们已经处于新一轮加息周期中或前夜》` |
| [[综合整治非法跨境证券期货基金经营活动实施方案]] | `《综合整治非法跨境证券期货基金经营活动实施方案》` |

#### 实体别名修复 (5)
- [[LudwigErhard]] → `Ludwig Erhard` (含空格)
- [[国务院]] → `中华人民共和国国务院`, `中华人民共和国中央人民政府`, `中央人民政府`
- [[研究：美元如何收割新兴市场]] → `研究：美元如何收割新兴市场（增强版）`
- [[共建一带一路]] → 修复 `共建"一带一路"` YAML 引号问题
- [[金砖支付系统]] → `BRICS Pay`

#### 危机系列别名修复 (2)
- [[1998香港金融保卫战]] → `1998 香港金融保卫战`
- [[2023年SVB危机]] → `2023 svb危机`

#### 表格格式修复
- `美联储vs中国央行.md`: 全文件移除 wikilink 尾部 `\`，修复约 30 个死链

### Round 3 (This Session — 继续)
- 共建一带一路.md: YAML 别名修复（弯引号→直引号）— `[[共建"一带一路"]]` 死链 ✅
- 金砖支付系统.md: 新增别名 `金砖支付`
- `wiki/domains/中国经济` → 修正为 `[[中国经济]]`（巫师财经 source 页）

**剩余死链状态：** 52 个「真实缺失」中大部分为设计使然：
- 11 个 `raw/wechat/` 引用 → 源文件引用，故意为之
- 13 个 `cherry-picks#N` 节锚点 → 页面存在，锚点格式问题
- 7 个 `[[page#heading]]` 节锚点 → 需要验证 heading 是否匹配
- 5 个 `meta/` 交叉引用 → 旧 lint 报告引用，无需处理
- 3 个 `raw/zhihu/` / `wiki/domains/` → 已修 1 个，另 2 个旧报告引用
- 剩余少量高频缺页（Entities, Sources, Concepts, TARGET2 等）→ 🟢 LOW

#### 新增 Stub (30)
| 批次 | 页面 |
|------|------|
| 高频概念 | 金融安全, 产业升级, 城镇化, 伊朗制裁, Santiago Principle, 积极股东, 被动投资, ESG 投资, 主权基金透明度, 卡脖子技术 |
| 高频实体 | 港股 |
| 投资概念 | 估值修复, 天量成交, 期权策略, 财富转移效应, 货币政策策略 |
| 危机系列 | 1982 拉美债务危机, 1994 龙舌兰危机, 1998 俄罗斯卢布危机, 2001 阿根廷违约, 2014-2015 俄罗斯卢布危机, 2018-2019 土耳其阿根廷货币危机, 2022 俄罗斯制裁, 2026 新一轮美元紧缩周期与新兴市场 |
| 其他 | 2022-2024 美联储紧缩周期, 经常账户, 社会保障, 苹果涨价, 市场竞争, wiki-mode, wikilinks |
